...
impl FileConfig {
    fn load(explicit_config: Option<&str>) -> Result<Self, String> {
    ...
    fn string_owned(&self, keys: &[String]) -> Option<String> {
        keys.iter()
            .find_map(|key| self.values.get(&norm_key(key)).and_then(value_to_string))
    }

    fn list(&self, keys: &[&str]) -> Option<Vec<String>> {
...
fn parse_config_text(text: &str) -> Result<FileConfig, String> {
...
mod tests {
    use super::{
        parse_config_text, AgentInstructionConfig, CapabilityConfig, CompactionConfig, Config,
        ConfigOverrides, EffortCfg, FileConfig, FsConfig, ModelConfig, NetHttpConfig, Provider,
        RuntimeConfig, StoreConfig, ThinkingModeCfg,
    };
    use crate::core::thinking::ThinkingSupport;
